...
待删除扫描会同时输出 `SAFE` / `SENSITIVE` / `ADULT` 内容分级，识别色情、裸露、性暗示、暴力、血腥、自残、药物等可能的 18+ 或敏感内容。命中任何敏感标签的图片都会强制进入人工复核，不会仅因为内容敏感而自动批准删除。

扫描还会按项目比较原片内容哈希：同一内容的最早记录作为参照，后续副本标记为“疑似重复”。审核页面可通过类别下拉框单独筛选这些副本，再选择保留、单张提交删除或批量提交删除。
